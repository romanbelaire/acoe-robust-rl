...
def main(params):
    ...
    excluded_params = ['config_path', 'adv_config_path', 'out_dir_prefix', 'num_episodes', 'row_id', 'exp_id',
            'load_model', 'seed', 'deterministic', 'noise_factor', 'compute_kl_cert', 'use_full_backward', 
            'sqlite_path', 'early_terminate', 'cuda_id',  'compute_worst_q', 'load_q_model','blackbox', 'acoe_grad',
            'iteration',  'ref_model_list', 'attack_multiple_victims', 'attack_exp3', 'load_env', 'results_log', 'load_adv_model']
    ...
    def load_json_params(params, excluded_params, sarsa_params, imit_params):
        json_params = json.load(open('configs/'+params['config_path']))
        if json_params["method"]=='protected':
            for i in ["iteration","ref_model_list","attack_multiple_victims","attack_exp3","load_env","results_log"]:
                excluded_params.append(i)
        params = override_json_params(params, json_params, excluded_params + sarsa_params + imit_params)
        return params

    adv_override_params = copy.deepcopy(params)

    if params['config_path']:
        # Load from a pretrained model using existing config.
        # First we need to create the model using the given config file.
        params = load_json_params(params, excluded_params, sarsa_params, imit_params)
    ...
    if 'load_model' in params and params['load_model']:
        ...
        if 'load_adv_model' in params and params['load_adv_model']:
            adv_params = adv_override_params
            adv_params['config_path'] = params['adv_config_path']
            adv_params = load_json_params(adv_params, excluded_params, sarsa_params, imit_params)
            assert 'adv' in adv_params['mode'], "Adversary config must reference an adversarial training mode."
            a = Trainer.agent_from_params(adv_params, store=None)
            p.adversary_policy_model = copy.deepcopy(a.adversary_policy_model)
            p.params.ATTACK_ADVPOLICY_NETWORK = params['load_adv_model']
            del a
    ...
    if params['sarsa_enable']:
        ...
        for i in range(num_episodes):
            ...
            ep_length, ep_reward, actions, action_means, states, kl_certificates, worst_q, value = p.run_test(compute_bounds=params['compute_kl_cert'], use_full_backward=params['use_full_backward'], original_stdev=original_stdev)
            if i == 0:
                all_actions = actions.copy()
                all_states = states.copy()
            else:
...
def get_parser():
    ...
    parser.add_argument('--adv-config-path', type=str, default='', required=False,
                        help='json for adversary config')
    ...
    parser.add_argument('--load-adv-model', type=str, help='load a pretrained adversary model file', default='')
    parser.add_argument('--seed', type=int, help='random seed', default=1234)
    #parser.add_argument('--num-envs', type=int, help='number of parallel envs', default=1)
    ...
    parser.add_argument('--blackbox', action='store_true', help='use Blackbox attack')
    parser.add_argument('--acoe_grad', action='store_true', help='use gradient of Acoe model')

    #protected method configs
    parser.add_argument('--method', help='which paper is this experiment from?')

...
def main(params):
    ...
    if params['mode'] == "adv_ppo" or params['mode'] == 'adv_trpo' or params['mode'] == 'adv_sa_ppo' or params['mode'] == 'adv_pa_ppo' or params['mode'] == 'adv_acoe_ppo' or params['mode'] == "adv_destruct_ppo":
    ...
    if params['mode'] == "robust_q_ppo" or params['mode'] == 'adv_acoe_ppo' or params['mode'] == "adv_destruct_ppo":
        worst_q_table_dict = {
    ...
    def finalize_table(iteration, terminated_early, rewards):
        ...
        final_3_rewards = np.array(rewards)[-3:].mean()
        final_reward = np.array(rewards)[-1]
        final_dict = {
            'iteration': iteration,
            '5_rewards': final_5_rewards,
            '3_rewards': final_3_rewards,
            '1_rewards': final_reward,
...
def add_common_parser_opts(parser):
    ...
    parser.add_argument('--mode', type=str, choices=['ppo', 'trpo', 'robust_ppo', 'adv_ppo', 'adv_trpo', 'adv_sa_ppo', 'adv_pa_ppo', 'robust_q_ppo', 'adv_acoe_ppo', 'adv_destruct_ppo'],
                        help='pg alg')
    ...
    # PPO parameters
    parser.add_argument('--adam-eps', type=float, help='adam eps parameter')
    ...
    parser.add_argument('--attack-method', type=str, choices=["none", "critic", "random", "action", "sarsa", "sarsa+action", "advpolicy", "paadvpolicy", "action+imit", "gpd", "hidden"], help='adversarial attack methods.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--acoe', action='store_true', required=False)
    ...
    parser.add_argument('--blackbox', action='store_true', required=False)

    #protected method configs
    parser.add_argument('--method', help='which paper is this experiment from?')
    parser.add_argument('--iteration', type=int, default=1, help='number of iterations for our iterative methods')
    ...
    extra_params = ['config_path', 'out_dir_prefix', 'load_model', 'no_load_adv_policy', 
                    'adv_policy_only', 'deterministic', 'seed', 'norm_mode', 'ccer', 'acoe', 
                    'ccer_wppo', 'soarl', 'soft_q_u_regret', 'ccer_lambda', 'exp_id', 'acoe_advs', 'cuda_id', 'acoe_grad', 'blackbox', 
                    'method', 'iteration', 'ref_model_list', 'attack_exp3','load_env','results_log','attack_multiple_victims', 'num_envs']

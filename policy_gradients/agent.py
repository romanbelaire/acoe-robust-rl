...
from .steps import value_step, step_with_mode, pack_history, worst_q_step, acoe_step, ccer_q_step, soft_regret_q_u_step
from .logging import *
...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        self.lstm = self.params.HISTORY_LENGTH > 1
        ...
        if self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo' or self.MODE == 'robust_q_ppo' or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo":
            self.create_relaxed_model(time_in_state)

        # Minimax training
        if self.MODE == 'adv_ppo' or self.MODE == 'adv_trpo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo' or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo":
            ...
            if self.MODE == 'adv_pa_ppo' or self.MODE == 'adv_acoe_ppo' or (self.MODE == "adv_destruct_ppo" and not self.params.BLACKBOX):
            ...
            elif self.MODE == 'adv_destruct_ppo' and self.params.BLACKBOX:
            ...
            if self.params.METHOD == 'protected':
        ...
        print(self.policy_model.discrete, (self.AGENT_TYPE == "discrete"))
        assert self.policy_model.discrete == (self.AGENT_TYPE == "discrete")
        # Risk Q function optimization
        if self.MODE == 'robust_q_ppo' or self.MODE == 'radial_ppor' or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo":
    ...
    def create_relaxed_model(self, time_in_state=False):
        ...
        elif isinstance(self.policy_model, DiscPolicy) and self.ROBUST_PPO_METHOD == "sgld":
            self.relaxed_policy_model = None
            self.robust_eps_scheduler = LinearScheduler(self.params.ROBUST_PPO_EPS, self.params.ROBUST_PPO_EPS_SCHEDULER_OPTS)
            if self.params.ROBUST_PPO_BETA_SCHEDULER_OPTS == "same":
                self.robust_beta_scheduler = LinearScheduler(self.params.ROBUST_PPO_BETA, self.params.ROBUST_PPO_EPS_SCHEDULER_OPTS)
    ...
    def setup_stores(self, store):
        ...
        if self.params.MODE == 'robust_q_ppo' or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo":
            robust_q_cols ={
                'eps': float,
                'beta': float,
                'kl': float,
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        if collect_adversary_trajectory:
            ...
            assert self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo" or self.MODE == "adv_pa_ppo" or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo"
            if self.MODE == "adv_pa_ppo" or self.MODE == 'adv_acoe_ppo' or (self.MODE == "adv_destruct_ppo" and not self.params.BLACKBOX):
        ...
        is_advpolicy_training = self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo" or self.MODE == "adv_pa_ppo" or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo"
        ...
        for t in iterator:
            ...
            if is_advpolicy_training:
                ...
                if collect_adversary_trajectory or self.params.ADV_ADVERSARY_RATIO >= random.random():
                    if self.MODE == "adv_pa_ppo" or self.MODE == "adv_acoe_ppo" or self.MODE == "adv_destruct_ppo":
                        # Only attack a portion of steps.
                        old_states = last_states
                        ...
                        if self.params.BLACKBOX:
                            next_obs_perturbations = next_adv_perturbations.sign() * eps
                        else:
            ...
            if self.lstm:
                act_means, act_stdevs, hidden = self.policy_model.multi_forward(last_states, hidden=hidden)
                action_pds = (act_means, act_stdevs)
            else:
            ...
            if collect_adversary_trajectory:
                ...
                if self.params.ATTACK_METHOD == 'gpd' or self.params.ATTACK_METHOD == 'hidden':
                    old_action_pds = self.policy_model(old_states)
                    next_rewards = self.policy_model.calc_kl((next_action_means, next_action_stds), old_action_pds)
                    if self.params.ATTACK_METHOD == 'hidden':
                        old_states = old_states.requires_grad_(True)
                        with ch.enable_grad():
                            self.policy_model.zero_grad()
                            output, _ = self.policy_model(old_states)
                            loss = output.mean()
                            loss.retain_grad()
                            loss.backward()
                        grad = -loss.grad
                        eps = self.params.ATTACK_EPS if self.params.ATTACK_EPS != "same" else 0.1 #self.params.ROBUST_PPO_EPS
                        next_belief_dist = torch.sqrt(torch.mean((last_states - self.perturb_obs_fgsm(grad*eps, old_states))**2))
                        next_rewards -= next_belief_dist
                if self.MODE == "adv_destruct_ppo":
                    induced_kl = self.KL_dist(last_states, old_states, self.policy_model) #the info loss caused by the attack
                    next_rewards = induced_kl
                else:
        ...
        if infos.size > 0:
            _, ep_rewards = infos
            avg_episode_length = np.mean(infos[0])
            avg_episode_reward = np.mean(infos[1])
        else:
            ep_rewards = [-1]
    ...
    def perturb_obs_fgsm(self, directions, last_states):
        ...
        if self.lstm:
            old_action, old_action_stds, _ = self.policy_model(last_states, hidden=None)[0].detach()
            old_action = (old_action, old_action_stds)
        else:
        ...
        with torch.enable_grad():
            if self.lstm:
                new_action, new_action_stds, _ = self.policy_model(last_states.detach() + noise, hidden=None)[0]
                new_action = (new_action, new_action_stds)
            else:
                new_action = self.policy_model(last_states.detach() + noise)[0]
            if (self.params.MODE == 'adv_acoe_ppo' or self.params.MODE == "adv_destruct_ppo") and self.params.ACOE_GRAD:
                diff = self.q_model(ch.concat((last_states, new_action),dim=1)) - self.q_model(ch.concat((last_states, old_action),dim=1))
            else:
    ...
    def KL_dist(self, state1, state2, net, is_action=False):
        '''
        ...
        if self.params.ATTACK_METHOD == "critic":
            ...
            if steps > 0:
                ...
                if self.params.ATTACK_METHOD == "action+imit":
                    ...
                    if self.lstm:
                        old_action, old_stdev, _ = self.policy_model.multi_forward(last_states, hidden=None)
                    else:
                ...
                with torch.enable_grad():
                    for i in range(steps):
                        ...
                        if self.params.ATTACK_METHOD == "action+imit":
                            ...
                            if self.lstm:
                                action_change = (self.policy_model.multi_forward(states, history=None)[0] - old_action) / old_stdev
                            else:
            ...
            if steps > 0:
                ...
                with torch.enable_grad():
                    for i in range(steps):
                        ...
                        if self.lstm:
                            a, s, hidden = self.policy_model.multi_forward(states, hidden)[0]
                            actions = (a,s)
                        else:
            ...
            # Attack using a learned policy network.
            assert self.params.ATTACK_ADVPOLICY_NETWORK is not None
            if self.params.ATTACK_ADVPOLICY_NETWORK == 'exp':#load from experiment id
            ...
            # Clamp using tanh.
            if self.params.BLACKBOX:
                perturbed_states = last_states + (perturbations_mean.sign()*eps)
            else:
                perturbed_states = last_states + self.perturb_obs_fgsm(perturbations_mean, last_states)
            """
            adv_perturbation_pds = self.attack_policy_network(last_states)
            next_adv_perturbations = self.attack_policy_network.sample(adv_perturbation_pds)
            perturbed_states = last_states + ch.tanh(next_adv_perturbations) * eps
            """
            return perturbed_states.detach()
        elif self.params.ATTACK_METHOD == "hidden":
            # Attack using a learned policy network, using the CW-inspired hidden attack
            assert self.params.ATTACK_ADVPOLICY_NETWORK is not None
            if self.params.ATTACK_ADVPOLICY_NETWORK == 'exp':#load from experiment id
                self.attack_policy_network = self.adversary_policy_model
            if not hasattr(self, "attack_policy_network"):
    ...
    def collect_saps(self, num_saps, should_log=True, return_rewards=False,
        ...
        with torch.no_grad():
            ...
            if not test:
                ...
                if self.acoe and self.ACOE_ADVS:
                    concat_states = ch.cat((trajs.states, trajs.actions), dim=-1)
                    if self.lstm:
                        next_actions, next_stdev, hidden = self.policy_model.multi_forward(trajs.next_states, hidden=hidden)[0]
                        next_actions = (next_actions, next_stdev)
                    else:
    ...
    def take_steps(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True, saps2=None):
        if adversary_step:
            # collect adversary trajectory is only valid in minimax training mode.
            assert self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo" or self.MODE == "adv_pa_ppo" or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo"
        ...
        if (self.MODE == 'robust_q_ppo' or self.MODE == 'radial_ppor' or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo") and not adversary_step:
            ...
            if self.acoe:
                #q_model = ccer model
                q_loss = acoe_step(saps.states, saps.actions, saps.next_states, saps.not_dones, saps.rewards, q_model, target_q_model, policy_model, min_q_model, q_opt, self.params, self.robust_eps_scheduler, num_actions=self.NUM_ACTIONS, should_cuda=not self.params.cpu)
            ...
            elif self.MODE == 'robust_q_ppo': #Just wocar
        ...
        if (self.MODE == 'robust_q_ppo' or self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo' or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo") and not adversary_step and logging:
        ...
        if (self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo' or self.MODE == 'robust_q_ppo' or self.MODE == 'radial_ppor' or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo") and (isinstance(self.policy_model, CtsPolicy) or isinstance(self.policy_model, DiscCnnPolicy) or isinstance(self.policy_model, DiscPolicy)) and not adversary_step:
    ...
    def train_step(self):
        if self.params.METHOD == 'protected':
            self.train_step_impl = self.train_step_impl_protected
        if self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo" or self.MODE == 'adv_pa_ppo' or self.MODE == 'adv_acoe_ppo' or self.MODE == "adv_destruct_ppo":
            avg_ep_reward = 0.0
    ...
    def train_step_impl(self, adversary_step=False, increment_scheduler=True):
        ...
        return avg_ep_reward
    def train_step_impl_protected(self, adversary_step=False, increment_scheduler=True):
        '''
        Training implementation for 'adv_iterative_ppo' method
        Take a training step, by first collecting rollouts, then 
        calculating advantages, then taking a policy gradient step, and 
        finally taking a value function step.
    ...
    def run_test(self, max_len=2048, compute_bounds=False, use_full_backward=False, original_stdev=None, compute_worst_q=False):
        ...
        with torch.no_grad():
            ...
            print(msg % (np.mean(ep_reward), np.mean(ep_length)))
            if compute_bounds:
    ...
    def run_test_trajectories(self, max_len, should_tqdm=False, deterministic=False):
        ...
        for t in iterator:
            ...
            pairs = [
                (rewards, next_rewards),
                (actions, next_actions.view((1,1,-1))), # The sampled actions.
    ...
    def agent_from_data(store, row, cpu, extra_params=None, override_params=None, excluded_params=None, save_pt=False):
        ...
        if extra_params['mode'] == 'adv_acoe_ppo' or extra_params['mode'] == 'adv_destruct_ppo':
        ...
        agent_params['ref_model_list'] = [None]
        ...
        if agent_params['mode'] == 'adv_acoe_ppo' or agent_params['mode'] == 'adv_destruct_ppo':
    ...
    def agent_from_params(params, store=None):
        ...
        if params['history_length'] > 0:
            agent_policy = DiscLSTMPolicy if 'highway' in params['game'] else CtsLSTMPolicy
    ...
    def adversary_from_params(self, params, store=None):
        time_in_state = params['value_calc'] == 'time'
        adv_policy = policy_net_with_name(params['policy_net_type'])(self.NUM_FEATURES, self.NUM_FEATURES,
                                                self.INITIALIZATION,
                                                time_in_state=time_in_state,
                                                activation=self.policy_activation)
